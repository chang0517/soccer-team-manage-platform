import { listEvents, listTeams, listUsersByStatus, updateEvent } from "@/lib/db";
...
// 이상 도는 크론을 못 써서 이 시각 하나로 그날 경기를 다 잡는다. 모든 팀에
// 대해, 경기 당일 경기 종료(시작 시각 + MATCH_DURATION_HOURS) 후
// EQUIPMENT_REMINDER_DELAY_MINUTES가 더 지난 경기 중 비품(공가방1·공가방2·
// 물/음료·아이스박스) 담당자 4칸이 아직 다 안 채워졌으면, 아직 알림을 안
// 보낸 경기에 한해 그 팀 운영진 전원에게 입력 요청 푸시를 보낸다. "경기
// 당일"로 제한해서, 이 기능 도입 전부터 있던 과거 경기들이 한꺼번에 알림을
// 쏟아내는 일이 없게 한다.
export async function GET() {
  ...
  for (const team of teams) {
    const [events, adminUsers] = await Promise.all([
      listEvents(team.id),
      listUsersByStatus(team.id, "approved"),
    ]);
    ...
    for (const ev of events) {
      ...
      if (!allFilled) {
        await Promise.all(
          adminMemberIds.map((memberId) =>
            sendPushToMember(team.id, memberId, {
              title: "비품 담당자 입력",
      ...
      await updateEvent(team.id, ev.id, { equipmentReminderSent: true });
    }
  }
